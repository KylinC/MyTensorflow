...
#all_count is the total number of testdataset while the count is the correct one
all_count=0
count=0
for record in test_data_list:
    ...
    all_values=record.split(',')
    inputs=(numpy.asfarray(all_values[1:])/255.0*0.99)+0.01
    correct_result=all_values[0]
    test_result_list=n.query(inputs)
    test_result=test_result_list[0][0]
    test_result_number=0
    for i in range(10):
        if test_result_list[i][0]>test_result:
            test_result=test_result_list[i][0]
            test_result_number=i
    if(test_result_number==int(correct_result)):
        count+=1
    '''
    print(test_result_number)
    print(correct_result)
    print("@@@@")
    '''
print(count*1.0/all_count)
